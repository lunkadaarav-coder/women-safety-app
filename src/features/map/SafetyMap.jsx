...
import {
  ...
  Marker, Circle, InfoWindow, DirectionsRenderer
} from "@react-google-maps/api";
import { INCIDENT_TYPES, RISK_LEVELS } from "./incidents";
import { computeRiskScore, getRiskColor, getRiskLevel, formatRelativeTime } from "./riskEngine";
...
import { MOCK_SAFE_PLACES } from "./mockRouteData";
...
const VIEW_MODES = [
  ...
  { id: "routing",    label: "Safe Route",icon: "🛣️" },
];
...
// ─── Main Component ──────────────────────────────────────────────────────────
const SafetyMap = ({ googleMapsApiKey, initialViewMode = "heatmap" }) => {
  ...
  const handleMapClick = useCallback((e) => {
    if (reportMode) {
      ...
      if (!routeStart) {
        ...
        setRouteEnd(end);
        calculateSafeRoutes(routeStart, end);
      } else {
        setRouteStart({ lat: e.latLng.lat(), lng: e.latLng.lng() });
        setRouteEnd(null);
        setDirectionsResp(null);
  ...
  }, [reportMode, viewMode, routeStart, routeEnd]);

  // ── Route Calculation ──────────────────────────────────────────────────────
  const calculateSafeRoutes = (start, end) => {
    ...
    directionsService.route({
      ...
      if (status === window.google.maps.DirectionsStatus.OK) {
        ...
        result.routes.forEach((route, index) => {
          ...
          MOCK_SAFE_PLACES.forEach(place => {
            let minDistance = Infinity;
            path.forEach(point => {
              const dist = window.google.maps.geometry.spherical.computeDistanceBetween(point, new window.google.maps.LatLng(place.lat, place.lng));
          ...
          incidents.forEach(inc => {
             let minDistance = Infinity;
             path.forEach(point => {
                const dist = window.google.maps.geometry.spherical.computeDistanceBetween(point, new window.google.maps.LatLng(inc.lat, inc.lng));
                if (dist < minDistance) minDistance = dist;
             });
             if (minDistance < 200) score -= 100;
  ...
  return (
    <div className="sm-wrapper">
      ...
      {/* ── Routing Click Prompt ────────────────────────────────────────────── */}
      {viewMode === "routing" && (
        <div className="sm-click-prompt" style={{ background: "rgba(139,92,246,0.9)" }}>
          <span className="sm-prompt-dot" style={{ background: "#fff" }} />
          {!routeStart ? "Click map to set Start Point" : !routeEnd ? "Click map to set Destination" : "Route Generated. Click to reset."}
        </div>
      )}
      ...
      <div className="sm-main">
        ...
        <div className="sm-map-area">
          ...
          >
            ...
            {/* Routing Polyline and Safe Places */}
            {viewMode === "routing" && (
              <>
                {directionsResp && (
                  <DirectionsRenderer
                ...
                {routeStart && <Marker position={routeStart} icon={{ url: buildMarkerSVG("A", "#3b82f6", 32), scaledSize: new window.google.maps.Size(32, 32) }} />}
                {routeEnd && <Marker position={routeEnd} icon={{ url: buildMarkerSVG("B", "#ef4444", 32), scaledSize: new window.google.maps.Size(32, 32) }} />}
                
                {MOCK_SAFE_PLACES.map(place => {
                  let icon = "🛡️";
                  let color = "#3b82f6";
                  if (place.type === 'restaurant') { icon = "🍔"; color = "#f59e0b"; }
                  if (place.type === 'lighting') { icon = "💡"; color = "#eab308"; }
                  if (place.type === 'hospital') { icon = "🏥"; color = "#ef4444"; }
                  return (
                    <Marker
                      key={place.id}
                      position={{ lat: place.lat, lng: place.lng }}
                      icon={{
                        url: buildMarkerSVG(icon, color, 28),
                        scaledSize: new window.google.maps.Size(28, 28),
                        anchor: new window.google.maps.Point(14, 14),
                      }}
                    />
